...
class RoomService {
     ...
     * @parma {WebRTCService} webRTCService
     */
    ...
    createRoomAsCommon(ps, conn) {
        const players = ps.map((obj, i) => {
            if (i !== 0 && i !== (ps.length - 1)) {
                const pconn = this.webRTCService.connect(obj.key);
                return new this.playerService.createPlayer(obj.id, obj.key, pconn);
            } else if (i === 0) {
                return new this.playerService.createPlayer(obj.id, obj.key, conn);
            } else {
