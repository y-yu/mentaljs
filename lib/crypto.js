...
class CryptoService {
    ...
    getRandomPoint(curve, paranoia) {
        ...
        paranoia = paranoia || 0;
        const r = this.getRandomNumber(curve, paranoia);
     ...
     *
     * @param {sjcl.ecc.curve} curve
     * @param {number|sjcl.bn} n
     * @returns {sjcl.bn}
     */
    inverseNumber(curve, n) {
        if (!(curve instanceof sjcl.ecc.curve)) {
            throw new exception.InvalidCurve(curve);
        }
        ...
        if (typeof(n) === "number") {
            n = new sjcl.bn(n);
        } else if (!(n instanceof sjcl.bn)) {
            throw new exception.InvalidNumber(n);
        }

        return n.inverseMod(curve.r)
    }
    ...
    equalsPoint(p1, p2) {
        ...
        return sjcl.bitArray.equal(p1.toBits(), p2.toBits());
    }
